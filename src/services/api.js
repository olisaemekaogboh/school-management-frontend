...
export const resultAPI = {
  addOrUpdateResultDTO: (resultData) => {
    ...
    if (!resultData.subjectId) throw new Error("subjectId is required");

    return api.post(`/results/student/${studentId}`, resultData);
  },

  addOrUpdateResult: (studentId, subjectId, session, term, scores = {}) => {
    if (!studentId) throw new Error("studentId is required");
  ...
  setTermPrintableStatus: (studentId, session, term, printable, message) =>
    api.patch(
